...
#meter só um dataset com tempo e Antas
dataa<-cbind(data[10], data$`Antas-Espinho`)
...
O3 <- as.numeric(data$`Antas-Espinho`)
...
model_auto<-auto.arima(train_set,max.p=5, max.q=5) #ARIMA(2,1,0)(2,0,0)[24] , AIC=44446.35, BIC=44481.75
...
##treinar modelos
models_antas=vector("list", 180)
for(i in 1:180){
  try(models_antas[[i]]<-Arima(train_set, order = dt_params[i,1:3], seasonal = list(order=dt_params[i,4:6],period=24),lambda = NULL))
}
...
i=as.numeric(rownames(dt_params2)[which(dt_params2$aic<=44446.35)])
