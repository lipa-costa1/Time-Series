...
#meter só um dataset com tempo e Entrecampos
dataa<-cbind(data[10], data$Entrecampos)
...
O3 <- as.numeric(data$Entrecampos)
...
model_auto<-auto.arima(train_set,max.p=5, max.q=5) #ARIMA(5,1,0)(2,0,0)[24] , AIC=47725.43   AICc=47725.45   BIC=47782.07
...
##treinar modelos
models_entrecampos=vector("list", 180)
for(i in 1:180){
  try(models_entrecampos[[i]]<-Arima(train_set, order = dt_params[i,1:3], seasonal = list(order=dt_params[i,4:6],period=24),lambda = NULL))
}

models<-models_entrecampos
...
i=as.numeric(rownames(dt_params2)[which(dt_params2$aic<47725.43)])
...
modelo1<-models[[as.numeric(rownames(tabela_final))[21]]]
modelo2<-model_auto
