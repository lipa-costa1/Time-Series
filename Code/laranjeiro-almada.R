...
#meter só um dataset com tempo e Paio-Pires
dataa<-cbind(data[10], data$`Laranjeiro-Almada`)
...
O3 <- as.numeric(data$`Laranjeiro-Almada`)
...
model_auto<-auto.arima(train_set,max.p=5, max.q=5) #ARIMA(2,1,0)(1,0,0)[24], AIC=38142.33   AICc=38142.34   BIC=38177.73
...
##treinar modelos
models_laranjeiro=vector("list", 180)
for(i in 1:180){
  try(models_laranjeiro[[i]]<-Arima(train_set, order = dt_params[i,1:3], seasonal = list(order=dt_params[i,4:6],period=24),lambda = NULL))
}

models<-models_laranjeiro
...
i=as.numeric(rownames(dt_params2)[which(dt_params2$aic<38142.33)])
...
modelo1<-models[[as.numeric(rownames(tabela_final))[31]]]
modelo2<-model_auto
