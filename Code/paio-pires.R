...
dataa<-cbind(data[10], data$`Paio-Pires`)
...
##treinar modelos
models_paio=vector("list", 180)
for(i in 1:180){
  try(models_paio[[i]]<-Arima(train_set, order = dt_params[i,1:3], seasonal = list(order=dt_params[i,4:6],period=24),lambda = NULL))
}
...
i=as.numeric(rownames(dt_params2)[which(dt_params2$aic<37761.41)])
...
modelo1<-models[[as.numeric(rownames(tabela_final))[9]]]
modelo2<-model_auto
