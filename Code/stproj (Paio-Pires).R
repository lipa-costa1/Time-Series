...
#meter só um dataset com tempo e Restelo
dataa<-cbind(data[10], data$Restelo)
...
#tentar ir pelo stl
y.stl<-stlplus(data_ts,s.window="period")
plot(y.stl)

y_residuals <- y.stl$data[,4]
par(mfrow=c(1,1))
acf(y_residuals, lag=168)
...
#como nao conseguimos detetar a seasonality vamos fazer fit de um sarima
...
model_auto<-auto.arima(train_set,max.p=5, max.q=5) #ARIMA(4,1,1)(1,0,0)[24] ARIMA(p,d,q)(P, D, Q)[24] AIC=39052.14; BIC=39101.68
...
##treinar modelos
models=vector("list", 180)
for(i in 1:180){
   try(models[[i]]<-Arima(train_set, order = dt_params[i,1:3], seasonal = list(order=dt_params[i,4:6],period=24),lambda = NULL))
}
...
add_aic<-function(models, dt_params2){
  aic=rep(NA,180)
...
i=as.numeric(rownames(dt_params2)[which(dt_params2$aic<39052.14)])
...
modelo1<-models[[as.numeric(rownames(tabela_final))[6]]]
modelo2<-model_auto
